...
begin
    ...
    begin
        ...
        loop
            ...
            t := now - t;
            check_ref(t < warm_us * 2 us, "", "  WRONG WARM DURATION: EXPECTED AT MOST " & ds_to_string(2 * warm_us) & " US, GOT " & ds_to_string(t));
            t := now;
            wait until rising_edge(data_in) for start_us * 2 us;
            t := now - t;
            check_ref(t >= start_us * 1 us, "", "  WRONG DURATION OF START COMMAND: EXPECTED AT LEAST " & ds_to_string(start_us) & " US, GOT " & ds_to_string(t));
            check_ref(t < start_us * 2 us, "", "  WRONG DURATION OF START COMMAND: EXPECTED AT MOST " & ds_to_string(2 * start_us) & " US, GOT " & ds_to_string(t));
            data <= 'Z';
            cerr <= '0';
            dso  <= '0';
            val  <= (others => '0');
            ...
            print("DHT11: START SIGNAL RECEIVED - " & ds_to_string(now));
            print("  SENDING DATA " & ds_to_string(value));
            if tmp < 8 then
                print("  WITH CHECKSUM ERROR AT BIT #" & ds_to_string(tmp));
            end if;
    ...
    begin
        wait until data = '0';
        t := now;
        check_ref(t >= warm_us * 1 us, "", "  WRONG DURATION OF WARM-UP TIME: EXPECTED AT LEAST " & ds_to_string(warm_us) & " US, GOT " & ds_to_string(t));
        check_ref(t < warm_us * 2 us, "", "  WRONG DURATION OF WARM-UP TIME: EXPECTED AT MOST " & ds_to_string(2 * warm_us) & " US, GOT " & ds_to_string(t));
        loop
            wait until dso = '1';
            t := now;
            wait until data = '0';
